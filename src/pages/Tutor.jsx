...
import { subscribeToChatSessions, subscribeToSessionMessages, askAITutor, askAITutorEphemeral } from '@/services/aiTutorService';
import { BrainCircuit, Send, Sparkles, AlertCircle, RefreshCw, MessageSquare, Plus, Menu } from 'lucide-react';
import ScienceLoader from '@/components/ui/ScienceLoader';
import MagneticButton from '@/components/ui/MagneticButton';
import ReactMarkdown from 'react-markdown';
import remarkGfm from 'remark-gfm';

export default function Tutor({ hideHeader = false, contextualPrompt = "", ephemeral = false }) {
  ...
  // Real-time listener for Firestore Chat Sessions
  useEffect(() => {
    if (ephemeral || !user?.uid) return;
    const unsubscribe = subscribeToChatSessions(user.uid, (data) => {
      setSessions(data);
    });
    return () => unsubscribe();
  }, [user?.uid, ephemeral]);

  // Real-time listener for current session's messages
  useEffect(() => {
    if (ephemeral) return; // Messages are handled in state directly
    if (!user?.uid || !currentSessionId) {
      setMessages([]);
      return;
    }
    
    const unsubscribe = subscribeToSessionMessages(user.uid, currentSessionId, (history) => {
      setMessages(history);
    });

    return () => unsubscribe();
  }, [user?.uid, currentSessionId, ephemeral]);
  ...
  const handleSend = async (e) => {
    ...
    try {
      ...
      if (ephemeral) {
        ...
        const responseText = await askAITutorEphemeral(userQuery, messages);
        // Add AI response
        setMessages(prev => [...prev, { role: 'model', text: responseText }]);
      } else {
        const newSessionId = await askAITutor(user.uid, currentSessionId, userQuery, messages);
  ...
  return (
    <div className="flex-1 w-full h-[calc(100vh-80px)] md:h-full flex relative overflow-hidden bg-white dark:bg-[#09090b] animate-in fade-in duration-700">
      
      {/* Sidebar for Chat History */}
      {!ephemeral && (
      <div className={`absolute md:relative z-30 left-0 top-0 h-full w-64 bg-gray-50 dark:bg-[#111113] border-r border-black/5 dark:border-white/10 flex flex-col transform transition-transform duration-300 ${isSidebarOpen ? 'translate-x-0' : '-translate-x-full md:translate-x-0'}`}>
        <div className="p-4 flex items-center justify-between border-b border-black/5 dark:border-white/10">
          <button onClick={startNewChat} className="flex-1 mr-2 flex items-center justify-center gap-2 bg-purple-600 hover:bg-purple-700 text-white py-2 px-4 rounded-xl font-medium transition-colors shadow-sm">
            <Plus size={18} /> New Chat
          </button>
          <button className="md:hidden p-2 text-gray-500 hover:bg-gray-200 dark:hover:bg-gray-800 rounded-lg" onClick={() => setIsSidebarOpen(false)}>
            <Menu size={20} />
          </button>
        </div>
        <div className="flex-1 overflow-y-auto p-2 space-y-1">
          {sessions.length === 0 ? (
            <div className="text-center text-sm text-gray-500 p-4">No recent chats</div>
          ) : (
      ...
      <div className="flex-1 flex flex-col min-w-0 bg-white dark:bg-[#09090b]">
        ...
        <header className="h-16 shrink-0 border-b border-black/5 dark:border-white/10 px-3 sm:px-6 flex items-center justify-between bg-white/80 dark:bg-black/50 backdrop-blur-xl z-20">
           <div className="flex items-center gap-3">
              {!ephemeral && (
              <button className="md:hidden p-2 -ml-2 text-gray-600 dark:text-gray-300 hover:bg-gray-100 dark:hover:bg-gray-800 rounded-lg" onClick={() => setIsSidebarOpen(true)}>
                <Menu size={24} />
              </button>
              )}
